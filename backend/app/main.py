...
print(f"GEMINI_API_KEY loaded: {GEMINI_API_KEY[:4] if GEMINI_API_KEY else 'None'}...")
if GEMINI_API_KEY:
    try:
...
@app.post("/api/gemini")
async def generate_gemini(input_data: AlgorithmInput):
    # Check if we have a valid API key and model
    if not os.environ.get("GEMINI_API_KEY"):
        return {"error": "Gemini API key not configured", "response": "This is a fallback implementation. The actual response would be generated by the Gemini 2.0 model."}
    ...
    try:
        ...
        contents = [
            types.Content(
                role="user",
                parts=[
                    types.Part.from_text(text=input_data.code),
                ],
            ),
        ]
        ...
        # Collect all chunks to return as a single response
        full_response = ""
        ...
        print(f"Error with Gemini 2.0 API: {str(e)}")
        return {"error": str(e), "response": "Failed to generate response from Gemini 2.0 model."}
        ...
        while True:
            ...
            try:
                ...
                if message.get("action") == "analyze":
                    ...
                    await manager.send_message(json.dumps(result), client_id)
                elif message.get("action") == "gemini":
                    result = await generate_gemini(AlgorithmInput(**message))
                    await manager.send_message(json.dumps(result), client_id)
            except Exception as e:
